...
$query = "
    SELECT posts.postID, posts.content, posts.dateTime, posts.attachment, posts.userID, users.userName
    FROM posts
...
<html lang="en">
              ...
              <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div id="postCaption<?php echo $posts['postID'];?>">
                                <p class="card-text"><?php echo htmlspecialchars($posts['content']); ?></p>
                                
                                <?php
                                    if (isset($_SESSION['userID']) && $_SESSION['userID'] == 1 && isset($posts['userID']) && $posts['userID'] == $_SESSION['userID']) {
                                        echo '<button class="btn btn-sm btn-secondary" onclick="toggleEdit(' . $posts['postID'] . ')">Edit</button>';
                                    }
                                ?>
                            </div>

                            <div id="editForm<?php echo $posts['postID'];?>" style="display: none;">
                                <form action="edit.php" method="POST">
                                    <input type="hidden" name="postID" value="<?php echo $posts['postID']; ?>">
                                    <textarea name="newContent" class="form-control mb-2"><?php echo htmlspecialchars($posts['content']); ?></textarea>
                                    <button type="submit" name="btnEdit" class="btn btn-sm btn-primary">Save</button>
                                    <button type="button" class="btn btn-sm btn-secondary" onclick="toggleEdit(<?php echo $posts['postID']; ?>)">Cancel</button>
                                </form>
                            </div>
                            ...
                            <div class="comment">
                                ...
                                <?php 
                                $commentQuery = "
                                    SELECT comments.commentID, comments.content, comments.dateTime, users.userName
                                    FROM comments
  ...
  <script>
        function toggleEdit(postID) {
            var captionDiv = document.getElementById(`postCaption${postID}`);
            var editFormDiv = document.getElementById(`editForm${postID}`);
